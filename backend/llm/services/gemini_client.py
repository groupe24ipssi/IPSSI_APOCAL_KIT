...
class GeminiLLMClient(LLMClient):
    ...
    def generate_quiz(self, source_text: str, title: str, difficulty: str = "medium") -> list[dict]:
        raw = self._call_gemini(source_text, title, difficulty)
        return parse_and_validate_quiz(raw)

    # ----- internals -----

    def _call_gemini(self, source_text: str, title: str, difficulty: str) -> str:
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "system_instruction": {"parts": [{"text": build_system_prompt(difficulty)}]},
